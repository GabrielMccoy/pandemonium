...
pandemonium <- function(df, cov = NULL, is.inv = FALSE, exp = NULL, linked = NULL, linked.cov = NULL, linked.exp = NULL, group = NULL, label = NULL, user_dist = NULL, dimReduction = list(tSNE = tSNE, umap = umap), getCoords = list(normal = normCoords), getScore = NULL) {
  ...
  if (!is.data.frame(df)) {
    tryCatch(df <- as.data.frame(df), , error = function(e) {
      stop("df must be coercable as a data frame")
    })
  }

  if (!is.data.frame(linked) && !is.null(linked)) {
    tryCatch(linked <- as.data.frame(linked), , error = function(e) {
      stop("linked must be coercable as a data frame")
    })
  }
  ...
  if (!is.null(cov)){
    # symmetric
    if (!isSymmetric(cov)) {
      warn("the covariance matrix provided in cov should be symmetric")
    }

    # positive semi-definite
    if (any(eigen(cov,only.values = TRUE))<0) {
      warn("the covariance matrix provided in cov should be positive semi-difinite")
    }

    # invertible
    precision <-max((cov %*% solve(cov))-diag(x=1,nrow = ncol(cov),ncol = ncol(cov)))
    if (precision>1e-10){
      warn("the inverse provided covariance matrix in cov may not invert correctly")
    }
  }
  ...
  if (!is.null(linked.cov)){
    ...
    if (!isSymmetric(linked.cov)) {
      warn("the covariance matrix provided in linked.cov should be symmetric")
    }

    # positive semi-definite
    if (any(eigen(linked.cov,only.values = TRUE))<0) {
      warn("the covariance matrix provided in linked.cov should be positive semi-difinite")
    }

    # invertible
    precision <-max((linked.cov %*% solve(linked.cov))-diag(x=1,nrow = ncol(linked.cov),ncol = ncol(linked.cov)))
    if (precision>1e-10){
      warn("the inverse provided covariance matrix in linked.cov may not invert correctly")
    }
  }
  ...
  lab.choice <- if (is.null(label)) {
    c(choose = "", full.colnames)
  } else {
    c(choose = "", full.colnames, "label")
  }
  ...
  server <- function(input, output, session) {
    ...
    shiny::observeEvent(input$app.load,
      {
        ...
        if (is.inv & setequal(df.colnames, input$space1)) {
          rv$covInv1 <- covInv
        } else {
    ...
    shiny::observeEvent(
      c(
        input$metric, rv$coord1, rv$coord2,
    ...
    output$clusterstats <- shiny::renderPlot(
      {
        ...
        rv$cstats %>%
          tidyr::pivot_longer(
            cols = .data$within.cluster.ss:.data$dmin,
            names_to = "stat"
          ) %>%
          ggplot2::ggplot() +
          ggplot2::geom_line(ggplot2::aes(x = .data$k, y = .data$value)) +
          ggplot2::labs(x = "# clusters", y = "") +
          ggplot2::facet_wrap(~stat,
            ncol = 3, scales = "free_y",
            labeller = cstat_labeller()
          ) +
          ggplot2::theme_bw()
      },
      height = 600
    ...
    output$wc <- shiny::renderPlot(
      {
        ...
        plotWC(rv$space2, input$px, input$py, rv$value$is.interest, rv$benchmarks$id,
          rv$col, rv$cond,
          groups = rv$groups, pal = rv$pal, a = input$alpha, showalpha = input$plotHulls
        )
      },
      height = 325
    )
    output$sigbins <- shiny::renderPlot(
      {
        shiny::req(rv$load.app, rv$value)
        plotSigBin(
          rv$space2, rv$value$is.interest, rv$benchmarks$id, rv$value$bins,
    ...
    output$distText <- shiny::renderText(
      paste0(
        "Average distance: ", round(mean(as.vector(rv$d_mat)), 1),
    ...
    output$benchmarks.grouping <- DT::renderDT({
      ...
      dt <- rv$space2[group.benchmarks$id, ] %>%
        tibble::add_column(cluster = unique(as.character(rv$user.group))) %>%
        tibble::add_column(r = group.benchmarks$r) %>%
        tibble::add_column(d = group.benchmarks$d) %>%
      ...
      DT::formatRound(
        ...
        backgroundColor = DT::styleEqual(
          unique(as.character(rv$user.group)),
          unique(rv$pcol)
        )
      )
    })
    ...
    tour.data <- shiny::reactive({
      shiny::req(rv$coord1, rv$coord2)
        ...
        output$tableAB <- shiny::renderPlot(
          {
            ...
            table(groupsA, groupsB) %>%
              ...
              ggplot2::scale_fill_distiller(palette = "RdGy") +
