...
| "hist" | histograms of the within and between cluster distances | no additional required settings, however the following are optional:<br> `bw.filt=` either `"within"` or `"between"` or a vector of both to filter the columns of the subplots.<br> `gr.filt=` groups to be plotted in the rows of the subplots.|
...
`makeResults` takes all of the same parameters as `makePlots`, the settings that need to be provided are `k`, `linkage` and `metric`. The output is then passed to `makePlots` and the previous settings no longer need to be provided. 
