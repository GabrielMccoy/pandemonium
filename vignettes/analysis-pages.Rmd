...
# Overview
This vignette describes the Analysis page of the pandemonium app and each of the tabs that are in the page. This is to serve as a reference for understanding these pages and the controls and outputs each of them contain.


# Tabs

Once the app has been run and data loaded in the data page the app will open into the analysis page. This consists of tabs that contain different visuals and their options.
...
|this is an ordered dendrogram plot of the clustering| This is a plot of the clustering assignment with axis as selected in the side bar. Alpha hulls for each cluster will aslo be shown in selected|
| **Score values** | **Score Bins** |
| If `getScores` returns scores for each point these will be shown here on the axis as selected in the side bar | If `getScores` returns bins for each point these will be shown here on the axis as selected in the side bar 
...
This tab will show benchmark information for clusters and user assigned groupings if supplied. The first collumns of the table will be the linked space values of the benchmark points, then the cluster radius and diameter will be given, finally the score value of the benchmark point will be given if getScores returns a score.
...
The top output is a colour scaled scatter plot of each variable in the cluster space shown on the x-y axis from the selection on the input page. Here the colour gradient shows high to low values for each variable in space 1.
...
Below this plot is 3 settings:

 - Center: centres the coordintes by their mean.
 - Scale: sclaes the coordinates by their variation
...
| Colour | Clouring of points in the plot | Clustering or scores, bins if calculted by `getScores` or user defined grouping if given in data page|

The dimension reduction plots use `plotly` for their displays this allows for zooming, panning aswell as hovering over points. This hover will display the points coordintes in dimension reduced form aswell as the value used for colouring and the point label if provided in the data page.
...
|--------|---------------------------------|----------------|
| Colour | Clouring of points in the plot | Clustering or scores, bins if calculted by `getScores` or user defined grouping if given in data page|
| Data | Data used in plot | Choice of either space 1 or space 2 or up to 5 principle componants from the PCA of each space |
| Tour type | Used to define the tour path | Grand and Radial will define their respective tours, the other choices are projection pursuit indices.\n Radial, Spline and Anomaly tours are not defined for 3d views. |
| Use slice | Selection to turn view srom a regular scatter to slice view | slice width can be chosen if selected |
...
This tab is used to evaluate the number of clusters that should be used. The Following cluster statistics are evaluated between two and eight clusters:
